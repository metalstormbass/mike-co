    ...
    <script>
        ...
        async function loadComparisonData() {
            try {
                ...
                // Fetch both index pages with cache-busting and proper headers
                ...
                if (!origResponse.ok || !cgResponse.ok) {
                    throw new Error(`Failed to fetch scan results: Original=${origResponse.status}, Chainguard=${cgResponse.status}`);
                }

                const origText = await origResponse.text();
                const cgText = await cgResponse.text();

                // Extract scan data from script tags using regex
                const extractData = (html) => {
                    // Use greedy match to capture entire array
                    const match = html.match(/const scanData = (\[[\s\S]*?\]);/);
                    if (!match) {
                        console.error('Failed to match scanData in HTML');
                        return null;
                    }
                    try {
                        return JSON.parse(match[1]);
                    } catch (e) {
                        console.error('Failed to parse scanData JSON:', e);
                        return null;
                    }
                };
