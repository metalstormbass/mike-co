...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .view-toggle-container { display: flex; justify-content: center; margin-bottom: 30px; }
        .view-toggle-wrapper { background: #18181b; padding: 6px; border-radius: 12px; display: inline-flex; gap: 4px; border: 2px solid #27272a; }
        .view-toggle-btn { padding: 12px 32px; background: transparent; color: #71717a; border: none; border-radius: 8px; font-weight: 700; cursor: pointer; transition: all 0.3s; font-size: 1em; }
        .view-toggle-btn.active { background: #ef4444; color: white; box-shadow: 0 4px 12px rgba(239, 68, 68, 0.4); }
        .view-toggle-btn:hover:not(.active) { background: #27272a; color: #e4e4e7; }
        .view-toggle-btn.chainguard.active { background: #22c55e; box-shadow: 0 4px 12px rgba(34, 197, 94, 0.4); }
...
<body>
    <div class="container">
        ...
        <div class="view-toggle-container">
            <div class="view-toggle-wrapper">
                <button class="view-toggle-btn active" id="original-toggle-btn">
                    🐳 Original Images
                </button>
                <button class="view-toggle-btn chainguard" id="chainguard-toggle-btn">
                    🔒 Chainguard Images
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        async function loadChainguardVulnerabilities() {
            try {
                console.log('Loading Chainguard scan data for comparison...');

                const response = await fetch('../data/chainguard.json');
                if (!response.ok) {
                    console.warn('Chainguard data not available');
                    return;
                }
                ...
                for (const scan of scanData) {
                    ...
                    try {
                        ...
                        console.warn('Failed to load Chainguard scan for', imageName, err);
                    }
                }

                chainguardVulnerabilities = cgVulns;
                console.log('Chainguard vulnerabilities loaded:', cgVulns.length);
            } catch (error) {
                console.warn('Failed to load Chainguard data:', error);
            }
        }
        ...
        document.getElementById('original-toggle-btn').addEventListener('click', (e) => {
            if (!showChainguard) return; // Already on original view

            showChainguard = false;

            // Update button styles
            document.getElementById('original-toggle-btn').classList.add('active');
            document.getElementById('chainguard-toggle-btn').classList.remove('active');
        ...
        document.getElementById('chainguard-toggle-btn').addEventListener('click', (e) => {
            if (showChainguard) return; // Already on chainguard view

            showChainguard = true;

            // Update button styles
            document.getElementById('chainguard-toggle-btn').classList.add('active');
            document.getElementById('original-toggle-btn').classList.remove('active');
